...
#include "../../include/Utils/offsetAllocator.h"
#include <stdbool.h>
#include <stdlib.h>
...
static inline u32 lzcnt_nonzero(u32 v) {
#ifdef _MSC_VER
...
static inline u32 tzcnt_nonzero(u32 v) {
#ifdef _MSC_VER
    unsigned long retVal;
...
static const u32 MANTISSA_BITS = 3;
static const u32 MANTISSA_VALUE = 1 << MANTISSA_BITS;
static const u32 MANTISSA_MASK = MANTISSA_VALUE - 1;
static const NodeIndex NODE_UNUSED = 0xffffffff;
const u32 NO_SPACE = 0xffffffff;
...
static u32 insertNodeIntoBin(Allocator* allocator,
                             const u32 size,
                             const u32 dataOffset);

static void removeNodeFromBin(Allocator* allocator, const u32 nodeIndex);
...
u32 uintToFloatRoundUp(const u32 size) {
    u32 exp = 0;
    u32 mantissa = 0;
...
u32 uintToFloatRoundDown(const u32 size) {
    u32 exp = 0;
    u32 mantissa = 0;
...
u32 floatToUint(const u32 floatValue) {
...
void freeAllocation(Allocator* allocator, Allocation allocation) {
    ...
    u32 combinedNodeIndex = insertNodeIntoBin(allocator, size, offset);
...
StorageReportFull storageReportFull(const Allocator* allocator) {
    ...
    for (u32 i = 0; i < NUM_LEAF_BINS; i++) {
        u32 count = 0;
        u32 nodeIndex = allocator->m_binIndices[i];
